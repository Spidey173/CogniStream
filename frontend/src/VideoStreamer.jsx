...
/** Build the default WS URL from VITE_API_URL or the current page location */
function getDefaultWsUrl() {
  const apiUrl = import.meta.env.VITE_API_URL;
  const apiKey = import.meta.env.VITE_API_KEY || "dev-secret-api-key";

  if (apiUrl) {
    const wsProto = apiUrl.startsWith("https") ? "wss:" : "ws:";
    const cleanHost = apiUrl.replace(/^https?:\/\//, "").replace(/\/$/, "");
    return `${wsProto}//${cleanHost}/api/v1/stream?api_key=${encodeURIComponent(apiKey)}`;
  }

  const proto = window.location.protocol === "https:" ? "wss:" : "ws:";
  return `${proto}//${window.location.host}/api/v1/stream?api_key=${encodeURIComponent(apiKey)}`;
}
